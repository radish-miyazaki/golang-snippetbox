	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !bytes.Contains(body, tt.wantBody) {
				t.Errorf("want body to equal %q", tt.wantBody)
			}
		})
	}
}
...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Invalid email (incomplete domain)", "Bob", "bob@example.", "", csrfToken, http.StatusOK, []byte("This field is invalid")},
		...
		{"Short password", "Bob", "bob@example.com", "validPa$$", csrfToken, http.StatusOK, []byte("This field is too short (minimum is 10 characters)")},
		{"Duplicate email", "Bob", "dupe@example.com", "validPa$$word", csrfToken, http.StatusOK, []byte("")},
		{"Invalid CSRF Token", "Bob", "bob@example.com", "", "wrongToken", http.StatusBadRequest, nil},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			form := url.Values{}
